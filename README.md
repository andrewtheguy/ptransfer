...
- **Anonymous signaling is narrowly scoped (experimental)**: When enabled, the browser establishes Tor through Snowflake and carries its Nostr TLS WebSockets through Tor exits. There is no fallback to a direct relay connection. The Nostr relays do not learn that device's direct IP, but the app host, Tor bootstrap/Snowflake infrastructure, the eventual WebRTC peer, and any STUN service remain outside this protection. See [Anonymous Signaling](./docs/ANONYMOUS_SIGNALING.md).

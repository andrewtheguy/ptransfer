...
identifiers to pTransfer and is incompatible with earlier releases. `v0.0.34` removes implicit
Nostr client defaults: integrations must now provide both the relay list and signaling mode.
...
1. Scan the sender's QR code, or paste the PIN or code they sent you, and click "Receive". A PIN QR can also be scanned with the phone's own camera app, which opens this page with the PIN filled in. Before receiving a PIN you can open **Advanced options** and enable **Anonymous signaling**; it protects only the device it is enabled on, so each participant chooses it independently
...
- **Anonymous signaling is narrowly scoped (experimental)**: When enabled, the browser establishes Tor through Snowflake and reaches Nostr relays as onion services, over a separate relay pool that both the sender and the receiver must have chosen. There is no fallback to a direct relay connection. The Nostr relays do not learn that device's direct IP, but the app host, Tor bootstrap/Snowflake infrastructure, the eventual WebRTC peer, and any STUN service remain outside this protection. See [Anonymous Signaling](./docs/ANONYMOUS_SIGNALING.md).
...
- **Anonymous signaling** (experimental PIN Exchange option): Carries Nostr signaling over Tor without changing the encrypted WebRTC data path. It is slower and less reliable and never silently falls back to direct relay signaling.
...
- [Anonymous Signaling](./docs/ANONYMOUS_SIGNALING.md) - Tor/WASM signaling architecture, privacy boundary, and build details

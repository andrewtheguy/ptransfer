...
- **Tor Onion Service**: The sending tab publishes a v3 onion service of its own and mints a one-time password; the receiver needs only those two strings. No relay, no STUN, no direct connection between the two networks, and nothing published that could be correlated afterwards. Slower, capped at 1 MiB, and interoperable with [ptransfer-cli](https://github.com/andrewtheguy/ptransfer-cli)'s `tor` subcommands. See [Tor Transport](./docs/TOR_TRANSPORT.md)
...
7. For Tor Onion Service, pick which Snowflake bridge the tab reaches Tor through and click "Publish Onion Service". Bootstrapping Tor and publishing the descriptor takes a while — minutes on a first run — after which the page shows an `.onion` address and a one-time password. Send the receiver both; the service answers only while the tab is open. A selection over 1 MiB cannot be sent this way.
...
4. If it was an onion address, enter the one-time password the sender gave you and choose a bridge, then click "Connect over Tor"
...
- **Tor Onion Service**: Requires internet. The sending tab publishes a v3 onion service and mints a one-time password; the two peers meet at a rendezvous point inside the Tor network, so neither learns the other's address and no relay, STUN server, or direct connection is involved. The same SPAKE2 exchange PIN mode uses authenticates the password, and the file is encrypted again inside the circuit. Slower than the other two and capped at 1 MiB per transfer, with no resume. Deliberately outside the interop protocol, but interoperable with ptransfer-cli's `tor` subcommands — either side may be a browser tab or the CLI. See [Tor Transport](./docs/TOR_TRANSPORT.md).

...
2. **Single Transfer Protocol**: `src/lib/p2p-transfer.ts` is the only file-transfer implementation used once a WebRTC data channel or a Tor stream is open. Every mode uses its one-way protocol: 128 KiB AES-GCM chunks as fast as the transport takes them, an `end` with the chunk and byte counts, and an `abort` the sender sends with its reason; the receiver verifies on its own and hangs up.
...
sequenceDiagram
    ...
    else Direct connection fails and ordinary relay fallback is available
        Sender->>Relays: Encrypted manifest, control, and file-piece events
        Relays-->>Receiver: Encrypted file pieces
...
Relays carry no offer or answer in Code Exchange. The diagram shows the ordinary
fallback, where public Nostr relays may carry encrypted file pieces after the
direct WebRTC attempt fails. With the anonymous option, onion-service Nostr
relays carry only the encrypted control channel and a temporary sender onion
service carries the file.

**Requirements:**
- Each side needs either a camera or clipboard access to carry the offer and answer

**Network Requirements:**
- **With internet**: Can work across different networks when ICE finds a direct route; if it cannot, an eligible file up to 100 MiB can use the selected Nostr or Tor fallback
- **Without internet**: Ordinary Code Exchange can work when the devices are on the same local network (Wi-Fi, LAN, etc.); no relay fallback is available, and the anonymous option cannot be used
- **Not air-gapped**: Requires some network connectivity between devices

**How it works:**
- With internet: Google and Cloudflare STUN servers help discover direct ICE candidates. TURN is not configured; when restrictive NAT or firewall rules prevent a direct connection, an eligible Code Exchange can use its selected fallback instead.
...
Once a reliable, ordered, message-oriented transport is open — a WebRTC data channel for Nostr and Code Exchange, a framed onion stream for the Tor transport — every mode uses one shared file-transfer protocol, specified in [INTEROP_PROTOCOL.md §7](INTEROP_PROTOCOL.md#7-transfer). It is one-way: everything on the wire goes from the sender to the receiver, the sender is complete once its last byte has left its send buffer, and the receiver verifies what arrived on its own and hangs up. Whether the file reached the other person is for the two of them to confirm outside the program. Over WebRTC it runs as one client of the bidirectional data channel described under [WebRTC](#webrtc-srclibwebrtcts-and-srclibduplex-channelts), not as the channel's owner; the channel's other direction serves the signaling phase, not the transfer.
...
3. The sender is paced by the transport alone: each send resolves once the channel has room for more (sends wait while more than 1 MiB is buffered), and its progress bar follows what the channel has taken, not what has arrived. The receiver decrypts and stores each chunk in order as fast as its storage allows. Nothing comes back to open a window, so what a receiver has taken off the channel but not yet written waits in its memory — a browser data channel has no receive-side backpressure of its own — and a receiver whose storage is slower than the link holds the difference until it catches up. It holds at most `RECEIVE_BACKLOG_MAX_BYTES` (256 MiB) of that; past it the receiver gives up and closes the connection, a clean failure where holding on would be a crashed tab.
4. After the last chunk the sender sends `{"t":"end","chunks":n,"bytes":b}` carrying the wire (encoded) byte count, and is complete once it has left its send buffer. It keeps the connection up until the receiver hangs up or a linger window passes (`SENT_LINGER_MS`, 10 s on a data channel; 30 s on an onion stream), so its own close cannot cut off what the transport is still delivering.
5. Receiver validates both values against what it received and then lets the link go: from here the rest is storing what is in hand, so the sender's close, or an `abort` it sends as its user moves on, no longer fails the transfer, and the stall watchdog stops. It waits for all pending decryptions, verifies that the indices arrived exactly once in order, and checks the total decrypted wire byte count. Deflated payloads are inflated between decryption and storage (capped at the transfer size limit as a decompression-bomb guard), so the sealed payload is the original file.
6. Receiver finalizes its storage and closes the connection, which is the only thing the sender ever hears from it.
7. A sender that gives up — cancelled by its user, a source that fails, a transport that will not drain — sends `{"t":"abort","reason":"…"}` and the receiver stops at once with that reason (`TransferAbortedError`) instead of finding out from a timeout. A receiver that gives up — cancelled, a chunk that fails authentication or a protocol check, a full disk, an over-limit payload — just closes the transport. A transport that closes before `end` has arrived is a connection failure for the receiver, and one that closes with bytes still in the sender's buffer is one for the sender (`P2PConnectionError`).

Both sides run an idle/stall watchdog (`STALL_TIMEOUT_MS`, `60s`) instead of any overall wall-clock deadline, each measuring the other. The sender fails when the transport will not take the next chunk, or will not drain after `end`, for the window — a receiver that stopped reading; while it waits on a slow source it runs no clock. The receiver's window resets on every incoming message (armed once the transport opens, via `attach()`) and stops once `end` has checked out, so a sender that goes quiet mid-stream aborts the receive while slow storage after the end does not. A timeout on either side rejects with `P2PConnectionError`, which the UI treats as a connection failure.

The receiver rejects duplicate indexes, out-of-range indexes, malformed chunk lengths, transfers exceeding the application limit, and malformed final counts; once `end` has checked out it has stopped reading, so nothing sent after it is seen. Text that is not a transfer control message is left to whatever else shares the link.

### Tor Onion Transport (`src/lib/tor/`)

The same transfer protocol, over a Tor stream instead of a data channel. `TorFramedStream` restores the discrete binary/text messages the protocol needs (`[kind][length][payload]`) and serializes its frame writes, `createTorLink` turns it into the same push-fed link a data channel is — one read loop hands every frame to subscribers, and on the sender's side watches for the receiver's close — and above that `sendFileOverLink` and `createTransferReceiver` are the identical code the WebRTC path runs. One shared wire protocol with two transports is why the `TransferLink` interface exists.
...
The rendezvous event is **plaintext**: `transferId`, the sender's Nostr pubkey (checked against the event author), the blinded SPAKE2 element `pA`, a fresh per-rotation nonce, and relay hints. Nothing in it is PIN-testable, and the file metadata is absent because it travels inside the sealed confirm (see below). It uses regular kind 4243 so relays retain it long enough for a receiver that connects after publication to query it; a NIP-40 expiration tag requests deletion at the end of the PIN lifetime. The live handshake then runs over ephemeral kind-24243 events:
...
- **Guessing**: 40 bits, blind. The current sender UI has no attempt counter or rate limit; a mismatch remains retryable so a typo does not end the transfer. `CONFIRM_CODE_ENTRY_TIMEOUT_MS` bounds how long the UI accepts entries. Crockford Base32 normalization folds `I`/`L` → `1` and `O` → `0` so transcription slips do not read as mismatches.
...
- **Rotation Countdown**: A progress bar and an m:ss countdown under the PIN show the time until the next 2-minute rotation replaces it. The countdown tracks only the currently displayed PIN; the previous-bucket grace window remains an internal claim-validation detail.
...
- **Wait Backstop**: A muted footnote states that an unclaimed transfer stops automatically after 30 minutes. PIN rotation separately limits each displayed PIN's validity.
...
>
> **The returned response is the confirmation step.** Code Exchange has no separate short confirmation code: the sender confirms the receiver by scanning or pasting that receiver's response. Someone who captures the offer can build a valid answer, but cannot push it into the sender's page; it counts only when the sender accepts that response.
...
**Recipient confirmation.** The full response plays the same role for Code Exchange that the short confirmation code plays for PIN Exchange: it is a value produced by the receiver that the sender must accept before the transfer starts. Code Exchange lets the sender scan or paste that value instead of typing it. In either mode, producing the value proves possession of the matching session but does not by itself identify the person holding it; recipient selection comes from the sender accepting the value returned by the intended receiver.
...
- **Broadcast receive.** Every current subscriber sees every incoming message, so several clients share the channel without taking messages from each other. `onEnd` tells a client once when the channel can no longer carry anything, and `flush` resolves once every send made before it has left the buffer, so a finished sender can let the connection go without cutting off the tail.
...
The shared transfer protocol is one client of it, and the channel is its `TransferLink`. On the direct path it reserves binary messages for content chunks toward the receiver; its control messages are JSON objects typed by `t`, travelling the same way, and the receiver ignores text that is not one of the two it defines, so other text messages can travel beside a transfer in either direction.
...
8. Attempt the direct connection (30 s with an eligible fallback, 120 s without); on success receive over the data channel and hang up with the file, on failure run the offer's fallback (`receiveOverFallback`). No response has to be handed over by a person, so nothing is held on screen
...
8. On success, send 128 KiB chunks over the data channel as it takes them, then `end`; complete once they have left the send buffer, and linger until the receiver hangs up
...
8. On success, decrypt/authenticate incoming chunks into the adaptive receive sink, validate the sender's `end`, and close the connection once the file is stored
...
In PIN Exchange, the rendezvous payload is published as plaintext JSON. Encrypting it under a PIN-derived key would provide no meaningful confidentiality benefit and would reintroduce the offline guessing target the PAKE removes. It carries:
...
| Transfer completion | The sender's `end` inside the encrypted WebRTC data channel; the receiver checks it and hangs up, sending nothing back | Same transfer protocol | Sealed `done` control message after whole-file SHA-256 verification | The same `end` inside the authenticated Tor framed stream, answered by the receiver's close |
...
- **Flow and completion**: the sender is paced by the channel's send buffer alone and finishes with `end` carrying the chunk count and wire bytes, complete once it has left the buffer; the receiver verifies the chunk count, in-order index sequence, and final decrypted wire byte count, stores what it holds, and hangs up. Nothing travels back.
...
flowchart TD
    ...
    Chunks --> Write
    Write --> HangUp[end checks out: seal and hang up]
```

Both receive modes reject duplicate, out-of-order, malformed, and oversized encrypted frames as each arrives, then verify the chunk count and total wire bytes announced by `end` before sealing the file and hanging up. The metadata `fileSize` is only a progress hint, never a bound on the payload.
...
| Transfer stall | 60 seconds | Idle/stall window (`STALL_TIMEOUT_MS`), each side measuring the other. The receiver arms it when the transport opens (`attach()`), resets it on every message, and stops it once `end` has checked out; the sender runs it while the transport will not take the next chunk, or will not drain after `end`. A steadily-progressing transfer of any size never trips it; a peer that goes quiet aborts after this span. There is no overall transfer deadline, and a sender waiting on its own input runs no clock |
| Sender linger after `end` | 10 seconds (data channel), 30 seconds (onion stream) | The bytes left the sender's buffer, not its machine (`SENT_LINGER_MS`, `LINGER_TIMEOUT_MS`): the connection stays up until the receiver hangs up with the file, so the sender's close cannot cut off what the transport is still delivering |

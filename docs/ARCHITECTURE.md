...
1. **Claim (receiver → sender)**: carries the receiver's element `pB` in plaintext (the sender needs it to finish the PAKE before any key exists) plus a body sealed with the session claim key: `transferId`, the echoed sender nonce, a fresh receiver nonce, both peers' Nostr pubkeys, and the rendezvous transcript hash (see below). The receiver publishes one claim per rendezvous candidate it collected.
2. **Verify + lockout (sender)**: for each retained generation whose bucket is current-or-previous and whose verification budget is not exhausted, the sender finishes the SPAKE2 run against `pB` and tries the seal. A body that opens *and* matches the generation's nonce, the transfer id, the sender's own pubkey, the claim event's author, and the generation's transcript hash is proof the receiver knows a live PIN *and* is acting on the rendezvous this sender actually published. The bucket is checked again after the asynchronous verification so a boundary crossing cannot admit an expired claim. The **first verified claim locks the transfer** to that receiver: rotation stops, rendezvous publishing stops, retained PAKE secrets are wiped, and all other claims are ignored. Invalid claims are silently ignored (transfer tags are public, so treating them as fatal would allow trivial denial of service) — but each one burns a unit of the generation's `CLAIM_VERIFY_LIMIT` budget, which is the online-guessing meter.
...
- **Why nonces**: the sender nonce is fresh per rotation and the receiver nonce is fresh per claim, and both are bound into the transcript, so nothing captured replays across rotations, transfers, or directions (claim and confirm also use distinct HKDF labels and differ by their `type` field). The nonces carry this on their own: the receiver's ephemeral scalar is fresh per claim, but the sender's is not — see *SPAKE2 scalar reuse*.
...
- **Online guessing is metered**: an active attacker must publish a claim per guess and gets no feedback for failures. The sender verifies at most `CLAIM_VERIFY_LIMIT` (100) claims per generation against a 55⁵ ≈ 5.0×10⁸ space, so a full 30-minute session (≈15 generations) concedes ~1,500 guesses — a ~3×10⁻⁶ success probability, and a *successful* guess still stalls at the confirmation code. Exhausting a generation's budget stalls that generation (see *Availability Is a Non-Goal*); rotation mints a fresh one.
- **SPAKE2 scalar reuse (a known RFC deviation)**: the receiver picks a fresh ephemeral `y` per rendezvous candidate, but the sender picks one `x` per PIN generation, publishes `pA = x·G + w·M` once, and finishes that same run against every claim the generation receives — up to `CLAIM_VERIFY_LIMIT` times per 2-minute bucket. RFC 9382 §6 states that "Randomly generated values, e.g., x and y, MUST NOT be reused; such reuse violates the security assumptions of the protocol and results in significant insecurity." This deployment knowingly deviates, because `pA` is published before any claimant exists and each claim is sealed under a transcript committing to the `pA` its author saw — a fresh `x` per claim would require publishing a new rendezvous *after* the claim arrives and having the claimant re-derive against it, i.e. a third round trip driven by unauthenticated claim-intents. The cost reuse carries in RFC 9382's own flow does not arise here: there, `A` emits `cA = MAC(KcA, TT)` for an attacker-chosen `pB`, so a reused `x` yields many chosen-`pB` outputs under one scalar, whereas this sender emits nothing on a failed claim — the claim *is* the receiver's key confirmation, and the confirm is published only once one verifies. An attacker replaying against the reused `pA` learns exactly one bit per claim (confirm or silence), the same bit `CLAIM_VERIFY_LIMIT` already meters. Reuse also cannot be combined with small-subgroup or invalid-curve probing of `x`: P-256 has cofactor 1 and every incoming element is checked on-curve and non-identity first. Two honest limits: this is a deployment-specific argument, not a security reduction — SPAKE2's proof assumes fresh scalars per execution, so the protocol here rests on the absence of a known attack rather than on the RFC's guarantee; and it holds only while the sender stays silent on failure, so adding *any* response to an unverified claim (an error, a nack, a distinguishable timing path) would turn the reused `x` into a chosen-`pB` oracle and make this a real weakness.
- **The mirrored guess channel**: any PAKE lets a fake *sender* extract one guess per claim the receiver sends, by publishing forged rendezvous events matching the receiver's hint. The receiver caps this at `MAX_CLAIM_CANDIDATES` (8) claims per attempt — ~1.6×10⁻⁸ per attempt — and each forged candidate costs the attacker a published event.
...
`deriveConfirmationCode` (`kdf.ts`) is a short authentication string: HKDF-SHA256 over the SPAKE2 root with the public per-transfer salt, info label `secure-send:nostr-session:v3:confirmation` bound to the transfer id, both handshake nonces, the rendezvous transcript hash, and the metadata hash, truncated to 40 bits and encoded as 8 Crockford Base32 characters.
...
3. On each incoming claim, finish the PAKE against the claimant's element for each retained generation (budgeted by `CLAIM_VERIFY_LIMIT`) and try the sealed body; first verified claim locks the transfer (invalid claims are ignored)
...
3. Run the receiver side of the PAKE against each candidate (up to `MAX_CLAIM_CANDIDATES`) and publish one sealed claim each; wipe the PIN scalar
...
- **Sender-side (pre-transfer)**:
  - Only verify claims against retained PIN generations whose recorded bucket is the sender's current or immediately previous bucket (and whose `CLAIM_VERIFY_LIMIT` budget remains); recheck the bucket after opening the claim, and stop publishing and honoring PINs at the first verified claim and at the 30-minute backstop.

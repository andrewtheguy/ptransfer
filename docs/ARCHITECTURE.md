...
**What it does not close.** The returned response is Code Exchange's confirmation handoff, but it does not add a second secret. The offer remains the only secret gating the transfer, so whoever captures it can derive the same shared secret and produce a valid response too. The sender must therefore scan or paste the intended receiver's response. PIN Exchange's separate short confirmation code protects a different workflow in which the operator reads a second value from the receiver.

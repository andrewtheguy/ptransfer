...
the live interoperability tests against ptransfer-cli live here and are run from here: bun run test:live:webrtc for PIN Exchange over a real data channel, bun run test:live:tor for the onion transport. Both drive a sibling ptransfer-cli checkout, so run the matching one after changing anything the two implementations share

this repo has slow opt in tests, don't run `bun test` directly on normal flow when those opt in tests are not expected to run